...
class PlanningListView extends StatefulWidget {
  const PlanningListView({Key? key}) : super(key: key);

  @override
  PlanningListViewState createState() => PlanningListViewState();
}

class PlanningListViewState extends State<PlanningListView> with SingleTickerProviderStateMixin {
    ...
    return SizedBox(
      ...
      child: Consumer<PlanningProvider>(
        builder: (context, planningProvider, _) {
          ...
          if (planningProvider.planningState.progressState == 1) return const Center(child: CupertinoActivityIndicator());
